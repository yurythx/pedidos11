...
class ItemVendaSerializer(serializers.ModelSerializer):
    ...
    venda = serializers.PrimaryKeyRelatedField(queryset=Venda.objects.all(), write_only=True, required=False)
    
    class Meta:
        model = ItemVenda
        fields = [
            'id', 'venda', 'produto', 'produto_nome',
    ...
    def create(self, validated_data):
        ...
        venda = validated_data.pop('venda', None)
        if venda is None:
            venda_id = self.context.get('request').data.get('venda')
            venda = Venda.objects.get(id=venda_id)
        
        # Cria item
        item = ItemVenda.objects.create(venda=venda, **validated_data)
...
class VendaCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Venda
        fields = ['id', 'cliente', 'vendedor', 'tipo_pagamento', 'observacoes']
        read_only_fields = ['id']
