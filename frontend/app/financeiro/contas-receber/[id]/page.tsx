...
import { ContaReceber, StatusConta, TipoPagamento } from '../../../../src/types'

export default function DetalheContaReceberPage({ params }: { params: { id: string } }) {
  ...
  const fetchConta = useCallback(async () => {
    try {
      setLoading(true)
      const data = await request.get<ContaReceber>(`/api/contas-receber/${params.id}/`)
      setConta(data)
    } catch (err: any) {
  ...
  const handleBaixa = async (e: React.FormEvent) => {
    ...
    try {
      await request.post(`/api/contas-receber/${conta.id}/baixar/`, baixaData)
  ...
  const getStatusBadge = (status: StatusConta, estaVencida: boolean) => {
    if (status === 'PAGA') return <div className="badge badge-success gap-2 p-3"><CheckCircle className="w-4 h-4" /> Recebida</div>
        ...
        <div className="card bg-base-100 shadow-xl md:col-span-2">
          <div className="card-body">
            ...
            <div className="grid grid-cols-2 gap-6">
              {conta.venda_numero && (
                <div>
                  <span className="text-xs font-bold text-gray-500 uppercase block mb-1">Origem</span>
                  <Link href={`/vendas/${conta.venda}`} className="btn btn-xs btn-outline gap-1">
      ...
      {showBaixaModal && (
        <div className="modal modal-open">
          <div className="modal-box">
            <h3 className="font-bold text-lg mb-4">Receber Pagamento</h3>
            <form onSubmit={handleBaixa} className="space-y-4">
              ...
              <div className="bg-base-200 p-3 rounded-lg text-center">
                 <p className="text-xs text-gray-500 uppercase">Valor Final a Receber</p>
              ...
              <div className="modal-action">
                ...
                >
                  {baixaLoading ? <span className="loading loading-spinner"></span> : 'Confirmar Recebimento'}
                </button>
              </div>
            </form>
          </div>
