...
import { ContaPagar, Paginacao, StatusConta } from '../../../src/types'
...
export default function ContasPagarPage() {
  ...
  const fetchContas = useCallback(async (page = 1) => {
    try {
      setLoading(true)
      let url = `/contas-pagar/?page=${page}`
  ...
  const handleBaixarClick = (conta: ContaPagar) => {
  ...
  const handleConfirmBaixa = async (data: PaymentData) => {
    ...
    try {
        await request.post(`/contas-pagar/${selectedConta.id}/baixar/`, data)
        fetchContas(pagination.current_page)
        alert('Conta baixada com sucesso!')
    } catch (err: any) {
        alert(err?.message || 'Erro ao baixar conta')
  ...
  const getStatusBadge = (status: StatusConta, estaVencida: boolean) => {
    if (status === 'PAGA') {
      return <span className="badge badge-success gap-2"><CheckCircle className="w-3 h-3" /> Paga</span>
    }
    if (status === 'CANCELADA') {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-base-content">Contas a Pagar</h1>
          <p className="text-base-content/60">Gerencie suas despesas e pagamentos</p>
        </div>
        <div className="flex gap-2">
          <Link href="/financeiro/contas-pagar/nova" className="btn btn-primary">
                    ...
                    <tr key={conta.id} className="hover">
                      <td>
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-base-content/60">{conta.fornecedor_nome || 'Sem fornecedor'}</span>
                          {conta.numero_documento && (
                            <span className="text-xs text-base-content/60">Doc: {conta.numero_documento}</span>
                          )}
                        </div>
                      </td>
                      <td>
